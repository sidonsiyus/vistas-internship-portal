    ...
    <div className="space-y-6 font-sans transition-colors duration-200">
      ...
      <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800/80 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-3">
          <span className="text-base font-bold text-slate-900 dark:text-white">{monthYearLabel}</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {dates.map((item) => {
          ...
          let statusStyle = 'bg-white dark:bg-slate-800/80 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-sm text-slate-800 dark:text-slate-200';
          let badge = null;

          if (item.isSpecialWorkingWeekend && item.status === 'AVAILABLE') {
            badge = <span className="text-[10px] text-blue-700 dark:text-blue-400 font-semibold">Working {item.weekday}! ({item.slotsLeft})</span>;
          } else if (item.status === 'AVAILABLE') {
            badge = <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-semibold">{item.slotsLeft} slots open</span>;
          } else if (item.status === 'LIMITED') {
            badge = <span className="text-[10px] text-amber-800 dark:text-amber-400 font-semibold">Only {item.slotsLeft} left!</span>;
          } else if (item.status === 'FULL') {
            statusStyle = 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed';
            badge = <span className="text-[10px] text-rose-600 dark:text-rose-400 font-semibold">Fully Booked</span>;
          } else if (item.status === 'CLOSED_TODAY') {
            statusStyle = 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed';
            badge = <span className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">Hours Passed</span>;
          } else if (item.status === 'HOLIDAY') {
            statusStyle = 'bg-slate-50/70 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed opacity-60';
            badge = <span className="text-[10px] text-amber-700 dark:text-amber-400 font-semibold">Holiday / Off</span>;
          } else if (item.status === 'UNAVAILABLE') {
            statusStyle = 'bg-slate-50/70 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-600 cursor-not-allowed opacity-60';
            badge = <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{item.weekday === 'Sat' || item.weekday === 'Sun' ? 'Weekend Off' : 'Day Off'}</span>;
          }

          if (isSelected) {
            statusStyle = 'bg-blue-50/80 dark:bg-blue-950/60 border-2 border-blue-600 text-blue-900 dark:text-blue-200 shadow-sm ring-2 ring-blue-500/10';
          }
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <span className="text-xs uppercase font-semibold text-slate-500 dark:text-slate-400">{item.daySubLabel}</span>
                {item.isToday && (
                  <span className="h-1.5 w-1.5 rounded-full bg-blue-600 dark:bg-blue-400 live-pulse" />
                )}
              </div>
              <span className="text-2xl font-bold font-mono tracking-tight text-slate-900 dark:text-white">{item.day}</span>
              {badge}
            </button>
          );
        })}
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center justify-between p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-400 gap-3">
