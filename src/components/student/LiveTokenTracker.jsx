  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 space-y-7 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="flex items-center gap-2 text-blue-600 text-xs font-bold uppercase tracking-wider mb-1">
            <span className="h-2 w-2 rounded-full bg-blue-600 live-pulse" />
        ...
        <form onSubmit={handleSearch} className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 font-mono focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 shadow-sm"
            />
          </div>
          <button
            type="submit"
      ...
      {myAppointment?.status === 'CALLED' && (
        <div className="bg-amber-50 text-amber-900 p-5 rounded-xl border border-amber-300 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold tracking-tight text-amber-950">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl flex flex-col justify-between shadow-sm">
          <div>
            ...
            {currentlyServing ? (
              <div className="space-y-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900">{currentlyServing.studentName}</p>
                  <p className="text-xs text-slate-600">{currentlyServing.department}</p>
                  <p className="text-[11px] text-blue-700 font-medium mt-0.5">{currentlyServing.category}</p>
                </div>
              </div>
            ) : (
              <div className="py-5 text-center text-slate-400 space-y-1.5">
                <Users className="w-7 h-7 mx-auto text-slate-400" />
                <p className="text-xs font-medium text-slate-500">Desk Ready For Next Student</p>
              </div>
            )}
          </div>
          ...
          <div className="pt-3 mt-3 border-t border-slate-100 text-[11px] text-slate-500 flex items-center justify-between">
            ...
            <span className="font-mono text-emerald-600 font-semibold shrink-0 ml-2">Live Sync</span>
          </div>
        </div>
          ...
          <div>
            ...
            {calledStudent ? (
              <div className="space-y-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-amber-800">{calledStudent.studentName}</p>
                  <p className="text-xs text-slate-600">{calledStudent.department}</p>
                  <p className="text-[11px] text-amber-700 font-medium mt-0.5 animate-pulse">Called • Approaching Desk</p>
                </div>
              </div>
            ) : nextTokenApt ? (
              <div className="space-y-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900">{nextTokenApt.studentName}</p>
                  <p className="text-xs text-slate-600">{nextTokenApt.department}</p>
                </div>
              </div>
            ) : (
              <div className="py-5 text-center text-slate-400 space-y-1.5">
                <CheckCircle2 className="w-7 h-7 mx-auto text-emerald-500" />
                <p className="text-xs font-medium text-slate-500">Queue is Clear</p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <span className="text-3xl font-bold text-slate-900 font-mono">{waitingList.length}</span>
              </div>

              <div>
                <span className="text-xs text-slate-500 block">Avg Consultation Pace</span>
                <span className="text-xs font-medium text-slate-700">~11 Minutes per student</span>
              </div>
            </div>
          </div>

          <div className="pt-3 mt-3 border-t border-slate-100 text-[11px] text-emerald-700 flex items-center gap-1.5">
      ...
      <div className="bg-white border border-slate-200 p-5 sm:p-6 rounded-xl space-y-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            <span className="text-[11px] font-semibold uppercase tracking-wider text-blue-600">YOUR TRACKED TOKEN</span>
            <div className="flex items-center gap-3 mt-1">
              {trackedToken ? (
                <TokenBadge tokenNumber={trackedToken} size="large" variant="blue" />
              ) : (
                <span className="text-sm text-slate-500 font-mono">No Token Tracked</span>
              )}
        ...
        {myAppointment ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            ...
            {/* Position Stat */}
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center space-y-0.5">
              <span className="text-[11px] text-slate-500 font-semibold uppercase">YOUR QUEUE POSITION</span>
              <div className="text-3xl font-bold text-blue-600 font-mono">{myPosition > 0 ? myPosition : '—'}</div>
              <p className="text-[11px] text-slate-500">{studentsAhead} student{studentsAhead === 1 ? '' : 's'} ahead of you</p>
            </div>

            {/* Estimated Wait Stat */}
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-center space-y-0.5">
              <span className="text-[11px] text-slate-500 font-semibold uppercase">ESTIMATED WAIT TIME</span>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1.5">
              <span className="text-[11px] text-slate-500 font-semibold uppercase block">LIVE ADVISORY</span>
              <p className="text-xs text-slate-700 leading-relaxed">
                {myAppointment.status === 'CALLED' ? (
                  <span className="text-amber-700 font-semibold">Please proceed to 7th Floor Staff Room immediately!</span>
                ) : myAppointment.status === 'IN_PROGRESS' ? (
                  <span className="text-blue-700 font-semibold">You are currently meeting the Internship Coordinator.</span>
                ) : myAppointment.status === 'COMPLETED' ? (
                  <span className="text-emerald-700 font-semibold">✓ Consultation completed. Have a great day!</span>
                ) : (
