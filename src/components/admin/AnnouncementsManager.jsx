  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 rounded-xl shadow-sm space-y-1">
          <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase">Total Posts</span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white font-mono">{totalCount}</div>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 rounded-xl shadow-sm space-y-1">
          <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 uppercase">Active for Students</span>
          <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 font-mono">{activeCount}</div>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 rounded-xl shadow-sm space-y-1">
          <span className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 uppercase">Company Replies</span>
          <div className="text-2xl font-bold text-blue-600 dark:text-blue-400 font-mono">{companyReplyCount}</div>
        </div>
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 rounded-xl shadow-sm space-y-1">
          <span className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 uppercase">Pinned to Top</span>
          <div className="text-2xl font-bold text-amber-600 dark:text-amber-400 font-mono">{pinnedCount}</div>
        </div>
      </div>

      {/* Controls Bar: Search & Filter Tabs */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-3.5 rounded-xl shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      ...
      <div className="space-y-3">
        {filteredList.length === 0 ? (
          ...
          filteredList.map(ann => (
            <div
              key={ann.id}
              className={`bg-white dark:bg-slate-900 border p-4 sm:p-5 rounded-xl shadow-sm transition-all ${
                !ann.isActive 
                  ? 'opacity-60 border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40' 
                  : ann.isPinned 
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="space-y-2 flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {ann.isPinned && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 px-2 py-0.5 rounded-full">
                    ...
                    {ann.type === 'COMPANY_REPLY' && ann.companyStatus && (
                      <span className="text-[10px] font-bold text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800/60 px-2 py-0.5 rounded-full">
                    ...
                    {ann.companyName && (
                      <span className="text-[11px] text-slate-700 dark:text-slate-300 font-semibold bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded flex items-center gap-1">
                    ...
                    {ann.companyLocation && (
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/60 px-2 py-0.5 rounded flex items-center gap-1 border border-slate-200/60 dark:border-slate-700/60">
                    ...
                    {ann.studentsIncluded && ann.studentsIncluded.length > 0 && (
                      <span className="text-[10px] font-bold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 px-2 py-0.5 rounded-full flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400 pt-0.5">
                      {ann.requestSentDate && (
                        <span className="flex items-center gap-1">
                          ...
                          <span>Request Sent: <strong className="text-slate-700 dark:text-slate-300">{ann.requestSentDate}</strong></span>
                        </span>
                      )}
                      {ann.replyDate && (
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                          <span>Reply Date: <strong className="text-slate-700 dark:text-slate-300">{ann.replyDate}</strong></span>
                        </span>
                      )}
                      {ann.emailReference && (
                        <span className="font-mono text-[10px] text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                  ...
                  {ann.studentsIncluded && ann.studentsIncluded.length > 0 && (
                    <div className="pt-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-semibold transition-colors"
                      >
                      ...
                      {expandedStudentsCardId === ann.id && (
                        <div className="mt-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 overflow-x-auto">
                          <table className="w-full text-left text-xs">
                            <thead>
                              <tr className="text-[10px] uppercase text-slate-400 dark:text-slate-500 border-b border-slate-200 dark:border-slate-700">
                            ...
                            <tbody className="divide-y divide-slate-200/60 dark:divide-slate-700 font-medium text-slate-700 dark:text-slate-300">
                              {ann.studentsIncluded.map((s, idx) => (
                                <tr key={idx} className="hover:bg-slate-100/50 dark:hover:bg-slate-700/50">
                                  <td className="py-1.5 font-mono text-[10px] text-slate-400 dark:text-slate-500">{idx + 1}</td>
                                  <td className="py-1.5 text-slate-900 dark:text-white font-semibold">{s.name}</td>
                                  <td className="py-1.5 font-mono text-blue-600 dark:text-blue-400">{s.registerNumber}</td>
                                  <td className="py-1.5 text-slate-600 dark:text-slate-400">{s.department || '-'}</td>
                                  <td className="py-1.5 text-slate-600 dark:text-slate-400">{s.year || ''} {s.section || ''}</td>
                                  <td className="py-1.5 text-slate-500 dark:text-slate-400 text-[11px]">{s.contact || '-'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                ...
                <div className="flex items-center gap-1.5 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100 dark:border-slate-800 justify-end">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg text-xs font-semibold border transition-all ${
                      ann.isPinned
                        ...
                        : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border-slate-200 dark:border-slate-700'
                    }`}
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors"
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/40 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-700 transition-colors"
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-10">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 text-blue-600 dark:text-blue-400">
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-5">
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-3 rounded-xl border text-xs font-semibold transition-all ${
                          formData.type === t.id
                            ...
                            : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-slate-600'
                        }`}
                      >
              ...
              {formData.type === 'COMPANY_REPLY' && (
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500"
                      />
                      {formErrors.companyName && (
                        <span className="text-[10px] text-rose-600 dark:text-rose-400 mt-0.5 block">{formErrors.companyName}</span>
                      )}
                    </div>

                    {/* Company Response Status */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Response Status</label>
                      <select
                        value={formData.companyStatus}
                        onChange={(e) => setFormData({ ...formData, companyStatus: e.target.value })}
                        className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:outline-none focus:border-blue-500"
                      >
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Company Location / Branch</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">HR / Coordinator Contact Email</label>
                      <input
                        type="text"
                    ...
                    {/* Date Request Sent */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Date Request Sent</label>
                      <input
                        type="date"
                    ...
                    {/* Reply Date */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Date Reply Received</label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Reference / Dispatch No.</label>
                      <input
                        type="text"
                    ...
                    {/* Target Department */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Eligible Department</label>
                      <select
                    ...
                    {/* Duration */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Internship Duration</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Reporting / Acceptance Deadline</label>
                      <input
                        type="date"
                  ...
                  {/* Required Documents */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Required Documents to Bring / Submit</label>
                    <input
                      type="text"
                  ...
                  <div className="pt-2 border-t border-slate-200 dark:border-slate-700 space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2 text-xs font-bold uppercase text-blue-700 dark:text-blue-400">
                          <Users className="w-3.5 h-3.5" />
                          <span>Students Included in This Company Request</span>
                          <span className="px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 text-blue-700 dark:text-blue-300 text-[10px] font-mono font-bold">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 text-xs font-semibold self-start sm:self-auto transition-colors"
                      >
                    ...
                    <div className="relative">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-8 pr-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500"
                        />
                      </div>
                          ...
                          {students
                            ...
                            .map(s => (
                              <button
                                ...
                                className="w-full text-left px-3 py-2 hover:bg-blue-50 dark:hover:bg-slate-700 flex items-center justify-between text-xs transition-colors"
                              >
                                <div>
                                ...
                                <span className="text-[10px] font-semibold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-800/60">
                                  + Select
                                </span>
                              </button>
                            ))}
                    ...
                    {formData.studentsIncluded.length === 0 ? (
                      <div className="p-4 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 text-center space-y-1">
                        <Users className="w-6 h-6 text-slate-400 dark:text-slate-500 mx-auto" />
                        <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">No students attached to this company reply yet.</p>
                      ...
                      <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                        {formData.studentsIncluded.map((std, idx) => (
                          <div
                            key={idx}
                            className="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 p-2.5 rounded-xl grid grid-cols-1 sm:grid-cols-12 gap-2 items-center"
                          >
                            <div className="sm:col-span-1 flex items-center justify-center font-mono text-[10px] text-slate-400 dark:text-slate-500 font-bold">
                              #{idx + 1}
                            </div>
                            ...
                            <div className="sm:col-span-3">
                              <input
                                ...
                                className="w-full px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500"
                              />
                            </div>
                            ...
                            <div className="sm:col-span-2">
                              <input
                                ...
                                className="w-full px-2 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
                              />
                            </div>
                  ...
                  {/* Eligibility */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Eligibility Criteria</label>
                    <input
                      type="text"
              ...
              <div className="space-y-3.5">
                ...
                <div>
                  ...
                  />
                  {formErrors.title && (
                    <span className="text-[10px] text-rose-600 dark:text-rose-400 mt-0.5 block">{formErrors.title}</span>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {formErrors.content && (
                    <span className="text-[10px] text-rose-600 dark:text-rose-400 mt-0.5 block">{formErrors.content}</span>
                  )}
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer text-xs select-none">
                  <input
                    ...
                    className="rounded bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-amber-500 focus:ring-0 w-4 h-4"
                  />
                  <div>
                    <span className="font-semibold text-slate-900 dark:text-white block">Pin this notice to top</span>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400">Always displays at the top of banner and feed</span>
                  </div>
                </label>
                ...
                <label className="flex items-center gap-2.5 cursor-pointer text-xs select-none">
                  <input
                    ...
                    className="rounded bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-0 w-4 h-4"
                  />
                  <div>
                    <span className="font-semibold text-slate-900 dark:text-white block">Active & Published</span>
                    <span className="text-[11px] text-slate-500 dark:text-slate-400">Visible to students immediately</span>
                  </div>
                </label>
